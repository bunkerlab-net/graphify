...
pub(crate) fn handle_stdout_error(e: &std::io::Error) -> ! {
    // `std` maps EPIPE (POSIX) and `ERROR_NO_DATA` (Windows) to `BrokenPipe`.
    // Windows also surfaces a write to a closed pipe as EINVAL, which slips
    // through as a raw OS error — treat it the same (matches graphify-py).
    let broken = e.kind() == std::io::ErrorKind::BrokenPipe || e.raw_os_error() == Some(22);
    if broken {
        std::process::exit(0);
    }
