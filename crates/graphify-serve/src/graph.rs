...
/// seeding: "how does the frontier cache work" seeds on `frontier`/`cache`, not
/// `how`/`the`/`work` (which prefix-match prose labels at the 100x tier).
/// Applied to query terms only — node text is never filtered, so a symbol
/// literally named `work` stays findable via explain/path.
///
/// Non-English question words are just as damaging (#1900): in a mostly-English
/// code corpus, German "wie"/"funktioniert" are rare, so they get HIGH IDF
/// weight and out-seed the actual content noun. So this also carries a curated
/// German set plus a trimmed French/Spanish/Portuguese/Italian set. Diacritics
/// are kept intact (the query tokenizer does not NFKD-strip). Collision
/// tradeoff: high-value "die"/"hat" are included (the all-stopword fallback and
/// unfiltered `find_node` path keep an English "die"/"hat" query workable), but
/// "war"/"bald" (German) and "comment"/"come"/"son"/"con"/"pour"/"des" are
/// deliberately omitted as too-common English/code terms.
