...
//!
//! Ports `graphify-py/graphify/validate.py`. See module docstring there.
...
///
/// Returns a list of error strings — empty means valid. Mirrors the Python
/// `validate_extraction` function byte-for-byte where practical.
#[must_use]
...
/// Validation error returned by [`assert_valid`].
#[derive(Debug, thiserror::Error)]
#[error("Extraction JSON has {} error(s):\n{}", errors.len(), errors.iter().map(|e| format!("  • {e}")).collect::<Vec<_>>().join("\n"))]
pub struct ValidationError {
    /// The list of error messages — same content as [`validate_extraction`].
...
/// [`validate_extraction`] would produce when the data violates the schema.
