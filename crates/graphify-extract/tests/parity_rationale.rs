//! Parity tests for rationale/docstring extraction.
//!
//! Mirrors `graphify-py/tests/test_rationale.py`: the v0.8.22 decorated-method
//! regression (#1050) and the Alembic autogenerated-file rationale behaviour
//! (module docstring suppressed, function docstrings still extracted).
#![allow(clippy::expect_used, clippy::unwrap_used)]

use std::collections::HashMap;
use std::path::{Path, PathBuf};

use graphify_extract::{Node, extract_js, extract_python};

/// Write `code` to `<dir>/sample.py` and return the path.
///
/// Mirrors `_write_py` (which `textwrap.dedent`s); we just supply an
/// already-dedented literal.
fn write_py(dir: &Path, code: &str) -> PathBuf {
    let p = dir.join("sample.py");
    std::fs::write(&p, code).expect("write sample.py");
    p
}

/// Regression for #1050: `@property` / `@staticmethod` / `@classmethod` methods
/// were emitted with a class-unqualified node id (e.g. `file_baz`) while the
/// rationale walker emitted the class-qualified id (`file_bar_baz`) as the
/// docstring edge's target. The mismatch caused `build_from_json` to drop the
/// `rationale_for` edge as dangling, orphaning the docstring node.
#[test]
#[allow(clippy::too_many_lines)] // single cohesive regression scenario; splitting would obscure it
fn decorated_method_node_id_is_class_qualified() {
    let tmp = tempfile::tempdir().expect("tempdir");
    let path = write_py(
        tmp.path(),
        r#"
class Bar:
    @property
    def baz(self) -> int:
        """Return the baz value because callers expect a cached integer."""
        return 1

    @staticmethod
    def helper() -> int:
        """A static helper documented for downstream callers."""
        return 2

    @classmethod
    def factory(cls) -> "Bar":
        """Construct a Bar via the canonical classmethod entry point."""
        return cls()

    def normal(self) -> int:
        """A normal instance method documented for comparison."""
        return 3
"#,
    );

    let result = extract_python(&path);
    let nodes_by_id: HashMap<&str, &Node> =
        result.nodes.iter().map(|n| (n.id.as_str(), n)).collect();

    // The plain method's id is the baseline: stem + class + name.
    let normal_ids: Vec<&str> = nodes_by_id
        .iter()
        .filter(|(_, n)| n.label == ".normal()")
        .map(|(id, _)| *id)
        .collect();
    assert_eq!(
        normal_ids.len(),
        1,
        "expected exactly one `.normal()` method node, got {normal_ids:?}"
    );
    let normal_id = normal_ids[0];
    assert!(
        normal_id.ends_with("_bar_normal"),
        "normal method id {normal_id} should be class-qualified"
    );

    // Each decorated method must share the same class-qualified id shape so the
    // `rationale_for` edge target matches the method node id.
    for decorated_name in ["baz", "helper", "factory"] {
        let want_label = format!(".{decorated_name}()");
        let matches: Vec<&str> = nodes_by_id
            .iter()
            .filter(|(_, n)| n.label == want_label)
            .map(|(id, _)| *id)
            .collect();
        assert_eq!(
            matches.len(),
            1,
            "expected exactly one `{want_label}` method node, got {matches:?}"
        );
        let method_id = matches[0];
        let want_suffix = format!("_bar_{decorated_name}");
        assert!(
            method_id.ends_with(&want_suffix),
            "decorated method id {method_id} should end with {want_suffix}"
        );
        // The buggy unqualified id must NOT also be present.
        let buggy = method_id.replace(&want_suffix, &format!("_{decorated_name}"));
        assert!(
            !nodes_by_id.contains_key(buggy.as_str()),
            "buggy unqualified id {buggy} should not exist alongside the class-qualified id"
        );
    }

    // Every `rationale_for` edge's target must resolve to an actual node (no
    // dangling edges into phantom unqualified ids).
    let rationale_edges: Vec<&_> = result
        .edges
        .iter()
        .filter(|e| e.relation == "rationale_for")
        .collect();
    for edge in &rationale_edges {
        assert!(
            nodes_by_id.contains_key(edge.target.as_str()),
            "rationale_for edge targets missing node id {}",
            edge.target
        );
    }

    // After build_from_json, each decorated-method docstring node must be
    // connected (degree > 0), not an orphan dropped from the graph.
    let extraction = serde_json::json!({
        "nodes": result.nodes,
        "edges": result.edges,
    });
    let graph = graphify_build::build_from_json(extraction, false, None).expect("build_from_json");
    let degree = |id: &str| -> usize {
        graph
            .edge_list
            .iter()
            .filter(|e| e.source == id || e.target == id)
            .count()
    };

    for decorated_name in ["baz", "helper", "factory", "normal"] {
        let want_label = format!(".{decorated_name}()");
        let method_id = nodes_by_id
            .iter()
            .find(|(_, n)| n.label == want_label)
            .map(|(id, _)| *id)
            .expect("method node present");
        let attached: Vec<&str> = rationale_edges
            .iter()
            .filter(|e| e.target == method_id)
            .map(|e| e.source.as_str())
            .collect();
        assert!(
            !attached.is_empty(),
            "no rationale_for edge found for `{want_label}` method"
        );
        for r_id in attached {
            assert!(
                graph.contains_node(r_id),
                "rationale node {r_id} missing from graph"
            );
            assert!(
                degree(r_id) > 0,
                "rationale node {r_id} for `{want_label}` is orphaned (degree 0) after build_from_json"
            );
        }
    }
}

/// An Alembic migration's module docstring is revision metadata, not
/// architectural rationale, so it is suppressed — but function docstrings inside
/// `upgrade()`/`downgrade()` must still be captured. Mirrors
/// `test_alembic_function_docstrings_still_extracted`.
#[test]
fn alembic_function_docstrings_still_extracted() {
    let tmp = tempfile::tempdir().expect("tempdir");
    let path = write_py(
        tmp.path(),
        r#""""Revision ID: 0002 Revises: 0001"""
revision = "0002"
down_revision = "0001"

def upgrade():
    """Add users table because auth was added in this release."""
    pass

def downgrade():
    """Drop the users table to reverse the migration."""
    pass
"#,
    );
    let result = extract_python(&path);
    let rationale: Vec<&Node> = result
        .nodes
        .iter()
        .filter(|n| n.file_type == "rationale")
        .collect();
    // Module docstring (revision metadata) is suppressed.
    assert!(
        !rationale.iter().any(|n| n.label.contains("Revision ID")),
        "Alembic module docstring must be suppressed"
    );
    // Function docstring inside upgrade() is still captured.
    assert!(
        rationale.iter().any(|n| n.label.contains("auth")),
        "function docstrings in autogenerated files must still be extracted"
    );
    // downgrade()'s docstring is captured too: autogen suppression is scoped to
    // the module docstring, not every docstring in the file.
    assert!(
        rationale.iter().any(|n| n.label.contains("reverse")),
        "downgrade() docstring must also be extracted in an autogenerated file"
    );
}
// ── JS/TS rationale + doc-reference extraction (6d3a6f1) ────────────────────

/// Write `code` to `<dir>/sample.ts` and return the path.
fn write_ts(dir: &Path, code: &str) -> PathBuf {
    let p = dir.join("sample.ts");
    std::fs::write(&p, code).expect("write sample.ts");
    p
}

fn nodes_of_type<'a>(r: &'a graphify_extract::FileResult, ft: &str) -> Vec<&'a Node> {
    r.nodes.iter().filter(|n| n.file_type == ft).collect()
}

#[test]
fn js_rationale_comment_extracted() {
    let tmp = tempfile::tempdir().expect("tempdir");
    let path = write_ts(
        tmp.path(),
        "// NOTE: must run before compile() or the linker will fail\nexport function build(): void {}\n",
    );
    let result = extract_js(&path);
    assert!(
        nodes_of_type(&result, "rationale")
            .iter()
            .any(|n| n.label.contains("NOTE")),
        "expected a rationale node mentioning NOTE"
    );
}

#[test]
fn js_block_comment_rationale_extracted() {
    let tmp = tempfile::tempdir().expect("tempdir");
    let path = write_ts(
        tmp.path(),
        "/**\n * WHY: retries are capped because the upstream rate-limits at 10 rps.\n */\nexport function fetchData(): void {}\n",
    );
    let result = extract_js(&path);
    assert!(
        nodes_of_type(&result, "rationale")
            .iter()
            .any(|n| n.label.contains("rate-limits")),
        "expected a block-comment rationale node mentioning rate-limits"
    );
}

#[test]
fn js_adr_reference_extracted() {
    let tmp = tempfile::tempdir().expect("tempdir");
    let path = write_ts(
        tmp.path(),
        "// Gateway pattern per ADR-0002; provider selection per ADR-0015.\nexport function route(): void {}\n",
    );
    let result = extract_js(&path);
    let labels: std::collections::HashSet<&str> = nodes_of_type(&result, "doc_ref")
        .iter()
        .map(|n| n.label.as_str())
        .collect();
    assert!(
        labels.contains("ADR-0002") && labels.contains("ADR-0015"),
        "labels: {labels:?}"
    );
    let cites = result
        .edges
        .iter()
        .filter(|e| e.relation == "cites")
        .count();
    assert_eq!(cites, 2);
}

#[test]
fn js_adr_reference_normalized_and_deduped() {
    let tmp = tempfile::tempdir().expect("tempdir");
    let path = write_ts(
        tmp.path(),
        "// See ADR-11 for the trust boundary.\n// ADR 0011 also governs the injection containment below.\nexport function guard(): void {}\n",
    );
    let result = extract_js(&path);
    let labels: Vec<&str> = nodes_of_type(&result, "doc_ref")
        .iter()
        .map(|n| n.label.as_str())
        .collect();
    assert_eq!(
        labels,
        ["ADR-0011"],
        "ADR-11 and ADR 0011 must collapse to one node"
    );
}

#[test]
fn js_adr_in_string_literal_not_extracted() {
    let tmp = tempfile::tempdir().expect("tempdir");
    let path = write_ts(
        tmp.path(),
        "export const banner = \"compliant with ADR-0099\";\n",
    );
    let result = extract_js(&path);
    assert!(
        nodes_of_type(&result, "doc_ref").is_empty(),
        "a doc-ref in a string literal (not a comment) must not be extracted"
    );
}
